...
> **Note:** For readability, this document presents the 27 RED categories across 35 user-facing topic headings. Several categories are expanded into multiple descriptive sections. The authoritative list of 27 **runtime slugs** (the exact `category=` values in `BLOCKED [RED]` error messages) is in [SECURITY.md](SECURITY.md). The user-facing heading names in this document (e.g., "File Deletion", "Disk Operations") are groupings for readability and do not appear verbatim in error messages.
