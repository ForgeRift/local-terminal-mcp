...
> **Note:** For readability, this document presents the 27 canonical RED categories across 35 user-facing topic headings. Several canonical categories (e.g., `code-exec`, `network-config`, `permissions`) are expanded into multiple descriptive sections. The authoritative 27-category slug list is in [SECURITY.md](SECURITY.md).
