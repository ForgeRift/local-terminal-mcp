...
- **F3 (MEDIUM)** GETTING_STARTED.md: "a small classification request" — singular. Pass 50 F5 corrected the same wording in manifest.json but GETTING_STARTED.md was missed. Changed to "two parallel classification requests (Layer 2 safety check and Layer 3 board review)."
- **F4 (MEDIUM)** CLAUDE_CONTEXT.md RED category table: `container` row listed `docker rm -f` and `docker system prune` as examples. Neither is blocked (`docker rm` is not blocked; `docker system prune` without `-af` is not blocked). Corrected to actual blocked patterns: `docker run`, `docker exec`, `docker build`, `docker push`, `docker pull`; noted `container-nuclear` for `docker system prune -af`; added explicit "docker rm / docker rmi NOT blocked" note consistent with COMMANDS.md.
- **F5 (LOW)** forgerift.io/index.html: AMBER tier badge and `<h4>` heading labelled "AMBER dry-run gate" — "gate" implies server enforcement that does not exist. Changed to "AMBER dry-run default" to match post-Pass-42 language used everywhere else.
- **F6 (LOW)** TROUBLESHOOTING.md: section heading "AMBER Dry-Run Gate Confusion" — same "gate" misnomer. Changed to "AMBER Dry-Run Default Confusion."
- **F7 (LOW)** manifest.json long_description: "A high-risk AI evaluation may independently block execution before the re-confirmation step" — "re-confirmation step" implies a server-enforced two-call gate. Changed to "regardless of dry_run value" to remove the implication.
- **F8 (LOW)** CLAUDE_CONTEXT.md `chaining` row: listed `cmd /c` and `bash -c` as pipe-to-shell examples without a leading pipe. Standalone `cmd /c` / `bash -c` fire `code-exec` (not `chaining`); the `chaining` slug for pipe-to-shell requires a leading `|` (pattern `\|\s*(bash|sh|cmd|...)`). Updated examples to `| cmd /c` and `| bash -c`; added note that standalone forms fire `code-exec`.
...
- **F3 (MEDIUM)** COMMANDS.md: `base64-exec` section listed `powershell -EncodedCommand` as an example. That pattern fires `code-exec` (BLOCKED_PATTERNS line ~137-163 via `powershell ... -Command/-EncodedCommand`), not `base64-exec`. Added clarifying note; moved the example context to accurate slug.
- **F4 (MEDIUM)** CLAUDE_CONTEXT.md: advanced env-vars table omitted two operator-tunable variables: `LAYER3_MODEL` (Layer 3 AI model selector; allowlist-validated) and `MCP_LOG_DIR` (audit log directory override; `/tmp` and world-writable paths rejected at startup). Added both rows to the table.
- **F5 (MEDIUM)** src/tools.ts → dist/tools.js: `find_files` tool description disclosed no operational limits. Added: max 500 results, max depth 8, 15-second deadline (partial results on timeout), symlinks and NTFS junctions skipped, sensitive paths silently filtered. Dist rebuilt.
- **F6 (LOW-MEDIUM)** src/tools.ts → dist/tools.js: `list_directory` description did not disclose that individual sensitive files (`.env`, SSH keys, credential stores) are silently filtered from listings even when the parent directory is accessible, and that output is truncated at 10,000 characters. Added disclosure. Dist rebuilt.
- **F7 (LOW-MEDIUM)** CLAUDE_CONTEXT.md: `lolbin` row listed `msiexec` but bare `msiexec` fires `code-exec` (BLOCKED_PATTERNS line 349 runs before HARD_BLOCKED_PATTERNS). The `lolbin` pattern at HARD_BLOCKED line 742 only fires for `msiexec ... /[qixa]` forms — but BLOCKED_PATTERNS runs first so it's unreachable for msiexec. Added clarifying note to the row.
- **F8 (LOW)** CLAUDE_CONTEXT.md: `direct-db` row said "anywhere in the command" but the pattern requires a trailing whitespace (`\s`). A keyword at end-of-line won't fire. Clarified: "followed by whitespace anywhere in the command."
- **F9 (LOW)** src/tools.ts → dist/tools.js: `search_file` description added in P47 ("non-recursive, immediate children") — dist confirmed accurate. No additional change needed.
