...
**Operating without the AI safety layer is "degraded mode."** The plugin's intended security model is the combination of Layer 1 (deterministic deny-list) **and** Layers 2–3 (AI classification). Operating with only Layer 1 active is supported and may be appropriate for some workloads, but it is materially weaker than the combined model and ForgeRift does not represent it as equivalent. Users operating in degraded mode accept the risk of any command Layer 1 alone fails to block; ForgeRift's liability for damage arising in this configuration is limited per the [MCP EULA](https://forgerift.io/legal/mcp-eula) Sections 6.1, 10, 11, and 12. Marketing language about "AI-assisted safety classification" describes the combined model and does not accurately describe the no-AI-key configuration.

**Deliberate weakening of safety controls is not supported.** Modifying, patching, or replacing the deterministic Layer 1 patterns (or the audit-log writer, the secret-scrubbing pipeline, the sensitive-file matcher, or the validators) to permit otherwise-blocked commands is a violation of the [MCP EULA](https://forgerift.io/legal/mcp-eula) Section 3 (Restrictions). Customers who do so void the liability protections the EULA otherwise provides. The plugin is open under MIT for source-code purposes; it is not open in the sense that operating a deployed plugin with the safety controls weakened is permitted under the deployed-product license.

**Binary-name canonicalization.** As of 2026-05, command argv is normalized through a `BINARY_ALIASES` map before pattern matching: `pwsh` → `powershell`, `nodejs` → `node`, `pip3` → `pip`, `cmd.exe` → `cmd`, `ncat`/`netcat` → `nc`, and similar variants resolve to their canonical forms. This closes a class of bypasses where alternative spellings of allowlisted binaries previously slipped pattern matches that targeted only the canonical name. See `src/tools.ts` for the full alias map.
...
Between v1.10.0 and v1.13.1, ForgeRift's internal adversarial review identified and resolved a series of edge cases in PowerShell argument parsing that could have allowed a command targeting a sensitive path to slip past the RED-tier blocker under specific argument-ordering conditions. All issues are resolved and the fixes are shipped in the current release (v1.13.1); earlier versions (v1.10.0�v1.10.2) carried the affected logic, with per-issue scope documented in the Known Scope notes below. The detailed engineering log below is preserved for security researchers and auditors.
...
| v1.10.3 | F-OP-72 / F-OP-74 / F-OP-79 | D10 PowerShell sensitive-path write matcher trailing-colon short-circuit closed: `-Destination:` with no inline value now returns `undefined` instead of matching the following token as a path (F-OP-72); sensitive-path regex patterns hoisted to module scope and unified so the same regexes apply consistently across all D10 matchers (F-OP-74); fail-closed guard extended to reject any UNC destination (`\\\\server\\share\\...`) that cannot be fully resolved at classification time, preventing bypasses via deferred path binding (F-OP-79). |
...
| v1.13.1 | - | S69 pre-marketplace audit close-out: F008 (plugin sends product_id; getMachineId fails closed on registry-read failure; execFileSync replaces execSync for the reg.exe probe) and F009 (VERSION constant aligned with package.json across src/index.ts, src/auth.ts, manifest.json, plugin.json, README, CLAUDE_CONTEXT.md, SECURITY.md). Tests 421/421 pass. |
...
**Secret redaction scope:** Secret redaction in command output covers token-shape patterns: known API key prefixes (`sk-ant-`, `sk-`, `ghp_`, `xoxb-`, `AKIA`, PEM headers, etc.). Key-value pairs such as `password=value` or `token=value` in command output are **not** redacted by this filter — only argument values written to the audit log undergo key-name-based redaction. Custom secrets that don't match the token-shape patterns will not be redacted from command output. Treat the audit log as potentially containing anything you passed to `run_command` and restrict access accordingly.
