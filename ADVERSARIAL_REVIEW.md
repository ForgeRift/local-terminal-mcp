...
**Attack vector:** `run_command: type C:\Users\alice\.ssh\id_rsa` — `type`, `Get-Content`, `findstr`, `gc`, `Select-String` all read credentials directly.
...
**Attack vector:** `rmdir /s /q C:\Users\alice\source` — `\brm\s/i` requires `rm` + whitespace, `rmdir` has `d` after `rm`.
...
**Attack vector:** `pwsh -c "Remove-Item -R -Fo C:\Users\alice\*"` — every `powershell`-only block is bypassed.
...
**Attack vector:** `\\?\C:\Users\alice\.ssh\id_rsa` (long-path prefix bypasses string match), `C:\Users\alice\.ssh\..\.ssh\id_rsa` (traversal), `notes.txt:hidden` (ADS).
...
| F-LT-85 | LOW | `sanitizeArgs` token-prefix list far smaller than tools.ts SECRET_OUTPUT_PATTERNS; key-name list missed credential/cookie/session/api_key | extended both: 30+ token prefixes (GitHub, Slack, AWS, …); key-name scan widened to match tools.ts list |

---

## Policy Audit — 2026-04-19 (S57, v1.8.2)

Non-adversarial audit — every tool description re-read against Anthropic Software Directory Policy §2.A–G, with particular attention to 2.B ("precisely match actual functionality, no unexpected functionality or undelivered features"). Four findings, all cosmetic / metadata. Closed in v1.8.2. No security implications, no code-path changes.

### F-AUDIT-1 (LT) — `list_directory` overclaim
**Issue:** Description said "local-terminal already has full Windows file system access." Code guards sensitive paths (`.ssh`, `.aws`, credential stores) via `isSensitiveFile`. "Full access" promises behavior intentionally withheld — Policy 2.B violation.
**Fix:** Reworded to "broad Windows file system access (sensitive paths like `.ssh`, `.aws`, and credential stores are guarded)."

### F-AUDIT-2 (LT) — `run_npm_command` annotation contradicted description
**Issue:** Description says "read-only npm commands." Annotation said `readOnlyHint: false`. Classifiers and reviewers treat annotations as ground truth; a contradiction with description fails 2.B.
**Fix:** Annotation changed to `readOnlyHint: true, destructiveHint: false`. All six approved sub-commands (list, outdated, audit, view, why, explain) query local package state with no mutation.

### F-AUDIT-3 (LT) — `run_git_command` mis-scoped `git fetch` as read-only
**Issue:** Description said "read-only git commands: status, log, diff, branch, fetch." `git fetch` updates `.git/refs/remotes/*` — mutates local git state even though working tree is untouched.
**Fix:** Reworded to "non-destructive git commands that don't modify the working tree." Annotation changed from `readOnlyHint: true` to `readOnlyHint: false` (non-destructive but not read-only). `fetch` stays in the allowlist.

### F-AUDIT-4 (LT) — `find_files` overclaim on uniqueness
**Issue:** Description said "This tool is the single source for file discovery on their machine." `list_directory` and `search_file` also discover files — the uniqueness claim overstated. Not a policy violation but cleaner to remove.
**Fix:** Replaced with "Call this tool directly for file-pattern discovery across a directory tree."

### Test coverage
173/173 passing. No test changes (no behavior changes).

### Notes — evaluated, not findings

- **"USE THIS — never ask…" directive language.** Tested against Policy 2.D (no coercing Claude into calling other Software). Our users explicitly installed local-terminal to automate these tasks — telling Claude to call the MCP rather than hand CLI commands back IS what the user wanted. Compliant ("requested and intended by a user"). Watch point: reviewers may find the tone heavy — soften to "prefer this tool over manual CLI steps" if feedback comes back.
- **`run_command` behavioral instructions.** Tested against Policy 2.G (no hidden/obfuscated/encoded instructions). Plain-English, safety-positive, in the visible description field. Compliant — arguably required for a safe escape hatch.
- **Copyright / trademark.** No third-party trademarks beyond factual tech names (PM2, git, npm, Windows). No findings.

---

## Token Frugality Audit — 2026-04-19 (S58, v1.8.3)

Non-adversarial audit — every tool output path re-read against Anthropic Software Directory Policy §5.B ("Output size should be commensurate with task complexity. Provide user options to exclude unnecessary text where appropriate."). Four of eight local-terminal tools needed the existing `truncateOutput()` wrapper applied. All four mechanical wraps shipped in v1.8.3.

### F-TOK-1 (LT) — `read_file` had no character cap
**Issue:** The 500-line clamp (`Math.min(500, end_line ?? 500)`) does not bound character count. Reading 500 lines of minified JS can exceed `MAX_CMD_OUTPUT_CHARS` (10,000).
**Fix:** Wrap return in `truncateOutput()`. One-line change.

### F-TOK-2 (LT) — `list_directory` had no entry cap
**Issue:** `readdirSync` → join output had no character cap. Listing `C:\Windows\System32` (~3,000 entries) or a `node_modules` root could climb past 10K chars.
**Fix:** Wrap return in `truncateOutput()`. One-line change.

### F-TOK-3 (LT) — `run_npm_command` had no output cap
**Issue:** `npm list` on a monorepo or `npm audit` with many vulnerable deps could produce >100K chars. No wrapper.
**Fix:** Wrap return in `truncateOutput(scrubSecrets(...))`. Symmetric with `run_git_command`. `scrubSecrets` runs first so redaction isn't cut off mid-pattern. One-line change.

### F-TOK-4 (LT) — `get_system_info` had no cap
**Issue:** Joined output of `ver`, `hostname`, `whoami`, `wmic logicaldisk`, `wmic OS`. Returned raw. Edge case on hosts with many drives/NICs.
**Fix:** Wrap return in `truncateOutput()`. One-line change.

### Notes — evaluated, not findings

- **`search_file`, `find_files`, `run_command`, `run_git_command`** already apply `truncateOutput()` (15/23 tools across both products were clean at first read; LT share was 5/8 pre-patch).
- **`find_files` 500-result hard cap** hides the cap from the caller — tracked as F-TOK-7, optional parity improvement. Not a frugality violation (the hard cap protects the budget); deferred.
- **`search_file` missing `context_lines` param** — tracked as F-TOK-8, parity gap with VPS. Deferred.

### Recommendation
Version bump: **patch (1.8.2 → 1.8.3)**. Mechanical truncate wraps only — no behavior change to bounded outputs, defensive cap on previously-unbounded outputs. Ship alongside VPS v1.8.4 frugality release.
