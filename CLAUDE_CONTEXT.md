...
local-terminal-mcp is a Claude Desktop extension distributed as a `.mcpb` package. Claude Desktop spawns `node dist/index.js` over stdio when the extension is enabled. There is no network listener, no auth token (the stdio channel is the trust boundary — it's only reachable by the Claude Desktop process that spawned it). Every `run_command` call passes through the following pipeline before executing:

- **Layer 1 (HARD_BLOCKED_PATTERNS):** 140+ hard-coded regex patterns across 27 categories — instant rejection, no AI consulted.
- **Layer 2 + Layer 3 (AI classification):** If an Anthropic API key is configured, two parallel Anthropic API calls run for every `run_command` invocation (not only AMBER-tier): Layer 2 is an AI pre-classifier and Layer 3 is a multi-perspective safety board review. Either may independently block the command. If the API call fails or no key is configured, both AI layers are skipped and the command falls through to Layer 1 static patterns plus the AMBER warning. `LAYER_STRICT_MODE=true` blocks on API failure instead of skipping.
- **BLOCKED_PATTERNS (RED scan):** After the AI layers, the full BLOCKED_PATTERNS array (covering `file-delete`, `code-exec`, `data-exfil`, `chaining`, and other categories) runs against the command.
- **AMBER classifier:** Deterministic pattern match — runs last in the pipeline. `dry_run=true` is the default for `run_command`; if the caller passes `dry_run=false` on the first call against an AMBER pattern, execution proceeds immediately (no session state enforces a two-call gate). The recommended flow is: first call with `dry_run=true` (the default) to see the preview, then re-call with `dry_run=false`.
